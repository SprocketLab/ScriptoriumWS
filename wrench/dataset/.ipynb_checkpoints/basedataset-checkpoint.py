...
class BaseDataset(ABC):
    ...
    def __init__(self,
        ...
        self.split = split
        self.path = path
        self.use_codexLF = use_codexLF
        self.pt_type = pt_type
        
        if path is not None and split is not None:
            self.load(path=path, split=split, use_codexLF=use_codexLF, pt_type=pt_type)
    ...
    def load(self, path: str, split: str, use_codexLF: bool, pt_type: str):
        ...
        if self.use_codexLF == True:
            logger.info(f'using label functions generated by codex ({pt_type})')
            data_path = path / f'{split}_{self.pt_type}_codex.json'
        else:
            logger.info(f'using label functions written by human')
    ...
    def get_uncovered_subset(self):
        idx = [i for i in range(len(self)) if np.any(np.array(self.weak_labels[i]) != -1) == False]
        return self.create_subset(idx)
